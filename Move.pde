...
class Move { 
  String jsonDate, path_date, place_date;
  int time_offset, date_offset, hours, mins, secs;
  ...
  Move(String jsonDate_) {
    jsonDate = jsonDate_;
    jsonFetch(jsonDate);
    // Parse JSON Response from Moves API
    ...
    for (int i = 0; i < moves.size(); i++) {
      ...
      for (int j = 0; j < segments.size(); j++) {
        ...
        //        println("Type: " + segment.getString("type") + " Start Time: " + segment.getString("startTime") + " End Time: " + segment.getString("endTime"));
        try {
          JSONArray activities = segment.getJSONArray("activities");
          for (int k = 0; k < activities.size(); k++) {
            JSONObject activity = activities.getJSONObject(k);
            // println("Activity: " + activity.getString("activity") + " Start Time: " + activity.getString("startTime") + " End Time: " + activity.getString("endTime") + " Duration: " + activity.getInt("duration"));
            // if (activity.getString("activity").equals("wlk") || activity.getString("activity").equals("run")) {
            // println("Distance: " + activity.getInt("distance") + " Steps: " + activity.getInt("steps"));
            // }
            JSONArray path = activity.getJSONArray("trackPoints");
            for (int l = 0; l < path.size(); l++) {
              JSONObject point = path.getJSONObject(l);
              //              println("- Lat: " + point.getFloat("lat") + " - Long: " + point.getFloat("lon") + " - Time: " + point.getString("time"));
              // build path array
        ...
        try {
          ...
          //          println("Start Time: " + segment.getString("startTime") + "* " + place.getString("name") + " * Type: " + place.getString("type") + " * ID: " + place.getInt("id"));
          JSONObject location = place.getJSONObject("location");
          //          println("* Lat: " + location.getFloat("lat") + " * Long: " + location.getFloat("lon"));
  ...
  void display() {
    ...
    for (int i=0; i<path_long.length; i++) {
      if ( i>0 && path_time[i] < TimeOfDay && path_time[i] > 0 ) {
        ...
        else if ( path_type[i].equals("run") && cycle.booleanValue() ) {
          stroke(255, 0, 255, 100);
          line(loc_x[i], loc_y[i], loc_x[i-1], loc_y[i-1]);
        }        
        else if ( path_type[i].equals("wlk") && cycle.booleanValue() ) {
          stroke(255, 255, 0, 100);
          line(loc_x[i], loc_y[i], loc_x[i-1], loc_y[i-1]);
        }        
        else if ( path_type[i].equals("trp") && cycle.booleanValue() ) {
          stroke(255, 40);
          line(loc_x[i], loc_y[i], loc_x[i-1], loc_y[i-1]);
        }
      }
  ...
  void jsonFetch(String fetchDate) {
    ...
    if ( !f.exists() || fetchDate.equals(thisDate) ) { // Call API if data doesn't exist
      try {
        ...
        String apiCall = "https://api.moves-app.com/api/v1/user/storyline/daily/" + fetchDate + "?trackPoints=true&access_token=" + accessToken;
